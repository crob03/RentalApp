...
### Decision 26: Sentinel `_userId = 0` for Self Mode in `UserProfileViewModel`
**Date**: 2026-05-04
**Area**: Architecture / Navigation

**Decision**: `UserProfileViewModel` supports two modes — self mode (viewing your own profile, navigated from the toolbar) and other-user mode (viewing a public profile, navigated from a tappable owner name). Rather than two separate routes or a nullable `int?` parameter, a single route is used and self mode is signalled by the absence of a `userId` query parameter. Because `_userId` is an `int` field, its default value of `0` serves as the sentinel: `_userId <= 0` means self mode; any positive value means other-user mode. A companion field `_resolvedUserId` is set after the async profile fetch in both branches, ensuring `FetchReviewsAsync` always receives a valid user ID regardless of mode.

**Alternatives considered**:
- **Nullable `int?` parameter** — `_userId = null` for self mode, `_userId = someId` for other-user mode. More semantically explicit than an int sentinel, but `IQueryAttributable.ApplyQueryAttributes` receives `object` values requiring a cast either way. No meaningful clarity advantage over `int` with a sentinel, and nullable int fields require null-checks throughout the ViewModel.
- **Two separate routes (`//profile/me` and `profile/{userId}`)** — cleanly separates the two modes at the routing layer. Rejected because it requires registering two routes, two `ShellContent` or `RegisterRoute` calls, and duplicating the navigation call sites in `AuthenticatedViewModel.NavigateToProfileCommand` and `ItemDetailsViewModel.ViewOwnerProfileCommand`. A single route with an optional parameter is simpler and consistent with how `ItemDetailsViewModel` receives its `itemId`.
