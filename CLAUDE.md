...
### RentalApp (MAUI UI)
MVVM pattern using `CommunityToolkit.Mvvm`. Views are XAML pages in `Views/`, bound to ViewModels in `ViewModels/`. `BaseViewModel` provides `IsBusy`, `Title`, and error handling. Services in `Services/` handle authentication (`IAuthService`) and navigation (`INavigationService`). Dependency injection is configured in `MauiProgram.cs`.

### RentalApp.Database (Data Access Layer)
EntityFrameworkCore with Npgsql (PostgreSQL). `AppDbContext` manages `User`, `Role`, and `UserRole` entities. Connection string is read from the `DB_CONNECTION_STRING` environment variable, falling back to `appsettings.json`. Passwords are hashed with BCrypt.
